...
if (!authStore.auth.isAuthenticated) {
  router.push("/login");
} else if (!authStore.auth.hasProfile || role !== "patient") {
...
<template>
  <v-img :src="backgroundImage" cover height="100%">
    <v-btn @click="router.go(-1)" size="large" class="mt-6 mx-6">
            ...
            <v-card-item>
              <v-sheet>
                <v-form @submit.prevent="submit">
                  <div class="text-subtitle-1 text-medium-emphasis">Doktor</div>
                  <v-select
