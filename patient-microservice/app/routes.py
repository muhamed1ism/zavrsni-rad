...
@bp.route('/create-patient', methods=['POST'])
@jwt_required()
def create_patient():
...
@bp.route('/update-patient', methods=['PUT'])
@jwt_required()
def update_patient():
    data = request.get_json()
    user_id = get_jwt_identity()
...
@bp.route('/delete-patient', methods=['DELETE'])
@jwt_required()
def delete_patient():
    user_id = get_jwt_identity()
    patient = db.session.query(Patient).filter(Patient.user_id == user_id).first()
...
@bp.route('/get-patient', methods=['GET'])
@jwt_required()
def get_patient():
    user_id = get_jwt_identity()
    patient = db.session.query(Patient).filter(Patient.user_id == user_id).first()
...
@bp.route('/get-patient/<int:patient_id>', methods=['GET'])
@jwt_required()
...
@bp.route('/get-patient/user/<int:user_id>', methods=['GET'])
@jwt_required()
